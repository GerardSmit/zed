...
impl AnyView {
    ...
    pub fn layer(self, style: StyleRefinement) -> ViewElement<AnyView> {
        ViewElement::new(self).layer(style)
    }
...
impl<T: Render> Entity<T> {
    ...
    /// Composite this entity as a cached GPU layer. Its subtree renders into its own offscreen
    /// texture only when its content is dirty (or scale changes); on a content-unchanged frame —
    /// notably every frame of a live window resize — the texture is re-composited (stretched to the
    /// new bounds) without re-running the view's render, layout, or paint. This decouples resize
    /// cost from the view's render cost, so any number of expensive panels cost ~nothing to resize.
    ...
    /// re-renders it crisp with correct hitboxes. Implies [`cached`](Self::cached) for layout.
    #[track_caller]
    pub fn layer(self, style: StyleRefinement) -> ViewElement<Entity<T>> {
        ViewElement::new(self).layer(style)
    }
}
...
impl<V: View> ViewElement<V> {
    ...
    /// Enable caching *and* composite this view's subtree through its own offscreen
    /// texture, laid out at `style`. See [`AnyView::layer`] for the tradeoffs.
    ///
    /// Crate-private for the same reason as [`cached`](Self::cached): the layer is only
    /// invalidated by [`Context::notify`] on the backing entity, so it requires an
    /// entity-backed view. Reach it through [`Entity::layer`] or [`AnyView::layer`].
    pub(crate) fn layer(mut self, style: StyleRefinement) -> Self {
        self.cached_style = Some(style);
        self.is_layer = true;
        self
    }
                ...
                window.with_element_state::<ViewElementState, _>(
                    ...
                    |element_state, window| {
                        ...
                        if self.is_layer && !cfg!(target_os = "macos") {
                            ...
                            if !needs_render && let Some(mut element_state) = element_state {
        ...
        if let Some(entity_id) = self.entity_id {
            ...
            window.with_rendered_view(entity_id, |window| {
                ...
                if self.is_layer && !cfg!(target_os = "macos") && !caching_disabled {
                    let layer_id = crate::LayerId(entity_id.as_u64());
