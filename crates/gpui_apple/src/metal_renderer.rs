...
impl MetalRenderer {
    ...
    fn draw_surfaces(
        ...
        for (index, surface) in surfaces.iter().enumerate() {
            let texture_size;
            // The CoreVideo texture wrappers own the Metal textures bound below, so they must
            // outlive the draw call that references them.
            ...
            match &surface.source {
                ...
                PaintSurfaceSource::Layer(id) => {
                    ...
                    command_encoder.set_render_pipeline_state(&self.layer_surfaces_pipeline_state);
                    texture_size = size(
